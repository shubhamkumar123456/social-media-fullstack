...
const sendMessage = async(req,res)=>{
    ...
    res.status(201).json({msg:"message send successfully"})
        ...
        res.status(500).json({msg:"error in send message ",error:error.message})
    }
}
...
const getMessage = async(req,res)=>{
    ...
    let conversation = await ConversationCollection.findOne({members:{$all:[_id, friendId]}}).populate({path:'members',select:'-password'}).populate('message')
